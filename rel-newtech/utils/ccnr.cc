...
bool 	ls_solver::local_search( const vector<char>* init_solution)
{
	...
	// {
		...
		if (0 == _unsat_clauses.size()) {return true;}	//1
		...
		{	
			...
			if (_unsat_clauses.size() < (unsigned long) _best_found_cost) {
...
void	ls_solver::initialize(const vector<char>* init_solution)
{
	...
	{
		//cout<<"ok\n";
		if (init_solution->size()!= (unsigned long) _num_vars) {cout<<"c Error: the init solution's size is not equal to the number of variables."<<endl; exit(0);}		
		for(v=1; v <= _num_vars; v++)
		{
